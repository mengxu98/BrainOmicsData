...
usage() {
  cat <<'EOF'
Usage:
  bash 06_upload.sh --host HOST --port PORT --user USER --password PASSWORD [options]
...
LOCAL_DIR="${LOCAL_DIR:-../../data/BrainOmicsData/ScienceDB}"
REMOTE_DIR="${REMOTE_DIR:-human_brain_age_interval_sc_snRNAseq_dataset}"
FTP_HOST="${FTP_HOST:-}"
FTP_PORT="${FTP_PORT:-}"
FTP_USER="${FTP_USER:-}"
...
if [[ "$DELETE_REMOTE_EXTRA" -eq 1 ]]; then
  ...
  for rel in \
    ...
    "figures/fig4_age_coverage.png" \
    "figures/fig4_age_coverage.tiff" \
    "figures/fig1_revised.pdf" \
    "figures/fig1_revised.png" \
    "figures/fig1_revised.tiff"; do
