...
#   ANALYSIS_DIR analysis directory containing metadata_working.rds
...
package="${BRAINOMICS_PACKAGE_DIR:-$repo_dir/results/run_root/package}"
cohort="${4:-}"
if [ -z "$cohort" ]; then
...
"$BRAINOMICS_PYTHON" "$stage_dir/percent_mito_inventory_sources.py" "$raw" "$qc/feature_inventory.tsv"
...
for dataset in "${r_object_datasets[@]}"; do
  ...
  "$BRAINOMICS_RSCRIPT" "$stage_dir/percent_mito_inventory_r_objects.R" "$raw" "$qc" "$dataset"
done

for dataset in "${matrix_datasets[@]}"; do
  [ -s "$qc/${dataset}_source_qc.tsv.gz" ] && continue
  log_message "source counts: $dataset (matrix)" --message-type info
  "$BRAINOMICS_RSCRIPT" "$stage_dir/percent_mito_calculate_matrix_sources.R" "$raw" "$qc" "$dataset"
done
...
if [ ! -s "$qc/Wang_2025_source_qc.tsv.gz" ] || [ ! -s "$qc/Velmeshev_2023_source_qc.tsv.gz" ]; then
  ...
  "$BRAINOMICS_PYTHON" "$stage_dir/percent_mito_calculate_h5ad.py" "$raw" "$qc"
fi

if [ ! -s "$qc/GSE168408_source_qc.tsv.gz" ]; then
  log_message "source counts: GSE168408 (original 10x H5)" --message-type info
  "$BRAINOMICS_PYTHON" "$stage_dir/percent_mito_calculate_gse168408.py" "$data_root" "$qc"
fi
...
if [ ! -s "$qc/gse296073_10x/GSE296073_source_qc.tsv.gz" ]; then
  ...
  "$BRAINOMICS_RSCRIPT" "$stage_dir/percent_mito_calculate_gse296073.R" "$raw" "$qc/gse296073_10x" GSE296073
fi

log_message "summarizing source inventories" --message-type info
"$BRAINOMICS_PYTHON" "$stage_dir/percent_mito_summarize_inventory.py" "$qc" "$cohort" "$work/percent_mito_source_inventory.tsv"

log_message "mapping source cells onto the released cohort" --message-type info
"$BRAINOMICS_RSCRIPT" "$stage_dir/percent_mito_map_to_cohort.R" "$analysis" "$data_root" "$qc" "$work"
test -s "$work/percent_mito_by_cell.tsv.gz"
