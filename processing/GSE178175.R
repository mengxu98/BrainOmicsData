...
res_dir <- check_dir("../../data/BrainOmicsData/processed/GSE178175/")

log_message("Start loading data...")
object <- readRDS(
  file.path(res_dir, "GSE178175_raw.rds")
)

metadata <- object@meta.data
metadata$Cells <- rownames(metadata)
metadata$Dataset <- "GSE178175"
...
saveRDS(
  object,
  file.path(res_dir, "GSE178175_processed.rds")
)
