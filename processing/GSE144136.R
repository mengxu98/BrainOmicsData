...
res_dir <- check_dir("../../data/BrainOmicsData/processed/GSE144136/")

log_message("Start loading data...")
object <- readRDS(
  file.path(res_dir, "GSE144136_raw.rds")
)

metadata <- object@meta.data
metadata$Cells <- rownames(metadata)
metadata$Dataset <- "GSE144136"
...
saveRDS(
  object,
  file.path(res_dir, "GSE144136_processed.rds")
)
