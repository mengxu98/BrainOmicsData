...
res_dir <- check_dir("../../data/BrainOmicsData/processed/AllenM1/")

log_message("Start loading data...")
object <- readRDS(
  file.path(res_dir, "AllenM1_raw.rds")
)

metadata <- object@meta.data
metadata$Cells <- rownames(metadata)
metadata$Dataset <- "AllenM1"
...
saveRDS(object, file.path(res_dir, "AllenM1_processed.rds"))
