...
out <- file.path(run, 'matrices/named24659/sources', ds)
dir.create(out, recursive=TRUE, showWarnings=FALSE)
stopifnot(!file.exists(file.path(out, 'SUCCESS.json')))
panel_file <- file.path(run, 'named_gene_panel_24659.tsv')
panel <- fread(panel_file); genes <- panel$Gene_Key
summary <- fromJSON(file.path(run, 'named_gene_panel_policy.json'))
stopifnot(length(genes) == summary$final_genes, !anyDuplicated(genes), summary$reference_cells == 2602031)
previous <- file.path(run, 'matrices/sources', ds)
previous_marker <- file.path(previous, 'SUCCESS.json')
input_mode <- 'cleaned_count_chunks'
if(file.exists(previous_marker)) {
  old <- fromJSON(previous_marker)
  old_file <- file.path(previous, 'object.rds')
  stopifnot(file.info(old_file)$size == old$bytes)
  message(Sys.time(), ' selecting named genes from completed object ', ds)
  object <- load_processed_object(old_file)
  counts <- LayerData(object, assay='RNA', layer='counts')[genes,,drop=FALSE]
  rm(object); gc(FALSE)
  input_mode <- 'completed_34380_source_object'
} else {
...
write_json(list(dataset=ds, reference_cells=length(ret), genes=length(genes), zero_count_cells=0,
    cell_membership_modified=FALSE, input_mode=input_mode,
