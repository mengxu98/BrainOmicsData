"""Assemble final source Seurat objects one at a time on 131; no repeat filtering."""
...
out = run/'run/named_source_assembly'; out.mkdir(parents=True, exist_ok=True)
...
script = run/'code/assemble_named_source.R'
summary = json.loads((run/'named_gene_panel_policy.json').read_text())
...
try:
    for ds in datasets:
        assert hashlib.md5(script.read_bytes()).hexdigest() == digest, 'Assembly script changed'
        destination = run/'matrices/named24659/sources'/ds; destination.mkdir(parents=True,exist_ok=True)
        marker = destination/'SUCCESS.json'
        status('ASSEMBLING_NAMED_SOURCE_OBJECTS', current_source=ds)
    ...
    (out/'summary.json').write_text(json.dumps(dict(reference_cells=2602031,genes=summary['final_genes'],
        ...
        next_stage='Combine named24659 source objects in reference_datasets order; validate the named panel before integration.'),indent=2)+'\n')
    status('NAMED_SOURCE_OBJECTS_COMPLETE', current_source=None)
