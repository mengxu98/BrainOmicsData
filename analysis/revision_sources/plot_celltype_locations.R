...
setDTthreads(4);root<-normalizePath(commandArgs(TRUE)[1]);out<-normalizePath(commandArgs(TRUE)[2]);source(file.path(root,'scripts/supervised_131/utils_original.R'))
...
source(file.path(out,'plotting/umap_style.R'))
base <- umap_theme
